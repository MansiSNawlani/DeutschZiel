...
DeutschZiel is a German exam-preparation tutor for learners sitting Goethe or
telc B1/B2. It gives Schreiben and Sprechen practice with feedback scored the
way the exam is scored, corrected at the learner's own level, and journalled so
that recurring mistakes become visible.

Read `CONTEXT.md` before naming anything: it is the binding glossary for the
domain. Read `docs/adr/0001-v0-v1-split.md` before proposing architecture: the
app you are working in is **v0**, a deliberately throwaway personal tool with no
backend, no database and no accounts. The full v1 plan in `project-list.md` is
frozen until after the exam.
...
An Expo app. There is no `dev`, `build` or `start` script in the Next.js sense;
Expo's CLI is the entry point and the app runs in a browser in v0.

- Dev server (web): `npx expo start --web` (http://localhost:8081)
- Dev server (all targets): `npm run start`
- Lint: `npm run lint`
- Typecheck: `npx tsc --noEmit`
- Static web export: `npx expo export --platform web` (output in `dist/`)

Use Chromium when exercising the journal folder or audio recording. Both the
File System Access API and `MediaRecorder` behave differently in other browsers.

There is no `test` command. Testing is opt-in and no runner is configured yet;
run `/tests` or `$tests` to add one and update this section with the real test
command.
...
There is no `Verify` command and no automatic GitHub checks. Run `/ci` or `$ci`
to set them up when you want them; they are not required.
